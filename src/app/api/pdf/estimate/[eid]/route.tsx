...
export async function GET(request: NextRequest, { params }: { params: Promise<{ eid: string }> }) {
  ...
  const buffer = await renderToBuffer(
    <QuoteDocument
      ...
      version={data.estimate.version}
      createdAt={data.estimate.created_at}
      lines={data.lines}
      totalPrice={data.estimate.total_price}
    />,
  );

  return pdfResponse(buffer, `견적서_${project?.name ?? "현장"}_v${data.estimate.version}`, request);
}
